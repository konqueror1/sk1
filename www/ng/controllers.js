...
.controller('CtrlMain', function($scope,$http,$timeout,$window,$filter) {
  ...
  $scope.tick = function(once,all) {
    ...
    $http.get('/gominer/f_status').success(function(d){
      if(d.info){
        angular.forEach(d.info, function(v,k) {Alertify.log.create(v.type, v.text);});
      }
      // Update status
      var poolStatsStrMap = [null, "NotReady", "Alive", "Sick", "Dead"]
      angular.forEach(d.status, function(v,k) {
        if(k == "pools"){
          for(var i=0; i<v.length;i++){
            v[i].status = poolStatsStrMap[v[i].status]
          }
        }
        $scope.status[k]=v;
      });
...
.controller('CtrlStatus', function($scope,$http,$timeout) {
  ...
  $scope.cgminer = function(command,parameter) {
    ...
    var execute = function(){
        $http.get('/gominer/f_miner?command='+(command || 'summary')+'&parameter='+parameter).success(function(d){
            if(d.info){
                angular.forEach(d.info, function(v,k) {Alertify.log.create(v.type, v.text);});
            }
            $scope.tick();
...
.controller('CtrlMiner', function($scope,$http,$timeout) {
 ...
 $scope.hostHardCtl = function(command) {
  ...
  $scope.poolActive = function(index) {
    angular.forEach($scope.pools, function(v,k) {if(k!=index) v.active=false});
    $scope.poolForm.$setDirty()
  };
...
.controller('CtrlBackup', function($scope,$http,$timeout) {
  ...
  $scope.restoring = 0;
  $scope.items = [
  {selected:true,name:'etc/gomienr.json'},
